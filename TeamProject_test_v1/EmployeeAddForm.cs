...
namespace TeamProject_test_v1
{
    public partial class EmployeeAddForm : Form
    {
        ...
        private void setDepartmentComboBox()
        {
            ...
            string query = "SELECT 부서명 FROM 부서 WHERE NOT 부서명 = '사장실'";
            if (Properties.Settings.Default.myID == "00000000")
            {
                query = "SELECT 부서명 FROM 부서";
            }
            DBManager.GetDBManager().OpenConnection();
            MySqlDataReader reader = DBManager.GetDBManager().SetQuery(query).ExecuteReader();
            while (reader.Read())
        ...
        private void setPositionComboBox()
        {
            ...
            DataSet ds = new DataSet();
            string query = "SELECT 직급 FROM 직급 WHERE NOT 직급 = '사장'";
            if (Properties.Settings.Default.myID == "00000000")
            {
                query = "SELECT 직급 FROM 직급";
            }
            DBManager.GetDBManager().OpenConnection();
            MySqlDataReader reader = DBManager.GetDBManager().SetQuery(query).ExecuteReader();
            while (reader.Read())
        ...
        {
            if (String.IsNullOrEmpty(textBoxName.Text) || String.IsNullOrEmpty(textBoxAge.Text) || String.IsNullOrEmpty(textBoxGender.Text) || String.IsNullOrEmpty(textBoxPhone.Text))
            {
        ...
        private void buttonAdd_Click(object sender, EventArgs e)
        {
            ...
            int 부서번호 = comboBoxDepartment.SelectedIndex;
            string 직급 = comboBoxPosition.SelectedItem.ToString();
            int 직급번호 = comboBoxPosition.SelectedIndex;
            if (comboBoxPosition.Items.Count != 3) 직급번호 += 1;
            ...
            string query = $"INSERT INTO 사원 VALUES('{textBoxNum.Text}', '{textBoxName.Text}', '{textBoxGender.Text}', '{textBoxAge.Text}', '{textBoxPhone.Text}', '{address}', '{textBoxEmail.Text}', '{textBoxAccount.Text}', {부서번호}, '{부서명}', {직급번호}, '{직급}')";
