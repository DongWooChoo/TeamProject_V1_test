...
namespace TeamProject_test_v1
{
    public partial class EmployeeUpdateForm : Form
    {
        ...
        private void setAdress()
        {
            ...
            foreach (Match match in matches)
            {
                if (!string.IsNullOrWhiteSpace(match.Value))
                {
                    if (count == 0)
                    {
                        textBoxAddress1.Text = match.Value;
                    }
                    if (count == 1)
                    {
                    ...
                    if (count == 2)
                    {
                        textBoxAddress3.Text = match.Value;
                    }
                }
                count++;
            }
        ...
        private void setInfo()
        {
            ...
            textBoxGender.Text = employeeInfo.성별;
            ...
            if (employeeInfo.사원번호 == Properties.Settings.Default.myID)
            {
        ...
        private void buttonConfirm_Click(object sender, EventArgs e)
        {
            ...
            if (comboBoxPosition.Items.Count != 3) 직급번호 += 1; //사장이 콤보박스에 없으면 +1이라는 뜻
            ...
            query = $"UPDATE 사원 SET 생년월일 = '{textBoxAge.Text}', 성별 = '{textBoxGender.Text}', 전화번호 = '{textBoxPhone.Text}', 주소 = '{address}', 이메일 = '{textBoxEmail.Text}', " +
