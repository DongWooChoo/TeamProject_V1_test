    ...
    public partial class 급여대장 : Form
    {
        ...
        private void work_month()
        {
            if (month_combobox.Items.Count == 0 && year_combobox.Items.Count == 0)
            {
                ...
                this.query = $"select distinct month(지급날짜) from s5584720.급여내역; select distinct year(지급날짜) from s5584720.급여내역;";
        ...
        {
            this.query = $"select concat(m.이름, '(', m.사원번호, ')') as '사원', m.직급, m.부서명, s.기본급, s.야간근로수당,s.연장근로수당,s.휴일근로수당,(s.기본급+s.야간근로수당+s.연장근로수당+s.휴일근로수당) as '지급합계', s.국민연금,s.국민건강보험,s.장기요양국민건강보험,s.고용보험료,(s.국민연금+s.국민건강보험+s.장기요양국민건강보험+s.고용보험료) as '공제액', s.실수령액, DATE_FORMAT(s.지급날짜,'%Y년 %m월 %d일') as '지급일' from s5584720.급여내역 s join s5584720.사원 m on s.사원번호=m.사원번호 where year(지급날짜)={Convert.ToInt32(year_combobox.SelectedItem)} and month(지급날짜)={Convert.ToInt32(month_combobox.SelectedItem)}";
        ...
        private void total_load()
        {
            ...
            total_member_dataGridView.Rows.Add("전체인원", $"{dtt.Rows[0][0]}명");
